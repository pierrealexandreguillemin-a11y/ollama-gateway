...
async function loadModels() {
  try {
    ...
    announceToScreenReader(
      `${models.length} modèles d'IA disponibles, orchestration multi-modèle incluse`
    );
  } catch (e) {
...
function renderMessages() {
  ...
  container.innerHTML = currentProject.messages
    .map(
      (msg, idx) => `
...
function toggleTheme() {
  ...
  btn.setAttribute(
    'aria-label',
    newTheme === 'dark' ? 'Basculer en mode clair' : 'Basculer en mode sombre'
  );
